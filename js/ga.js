...
function startGA()
{
	try
	{
		...
		if (finished)
		{
			...
			//If take out elitists, fun results of fluctuating lowest
			var popSize = parseInt($("input[name=popSize]").val()) || 200,
				generations = parseInt($("input[name=generations]").val()) || 500,
				elitists = parseInt($("input[name=elitists]").val()) || 2,
				alleles = parseInt($("input[name=alleles]").val()) || 7,
				minAllele = parseInt($("input[name=minAllele]").val()) || 1,
				maxAllele = parseInt($("input[name=maxAllele]").val()) || 7,
				extraAlleles = parseInt($("input[name=extraAlleles]").val()) || 2,
				mutationProb = parseInt($("input[name=mutationProb]").val()) || 20,
				statFreq = parseInt($("input[name=statFreq]").val()) || 10;

			wrongRoute = parseInt($("input[name=wrongRoute]").val()) || 2000;
			...
			//Seems like the more likely a mutation is, the better end solution comes up, here is a good setup:new Population(100,2,7,1,7,3,5).
			...
			for (var i = 0; i < generations; i++) 
			{
				try
				{
					...
					{
						var stats = populationObj.getStats();
						statHTML += "<p class='generation'>Generation: "+ (i + 1) + " " + stats +"</p>";
					}	
				}
				catch (ex)
				{
